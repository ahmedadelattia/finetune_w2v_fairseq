...
def parse_folder(folder,search_pattern = None, p2root = p2root, ext = ext, max_duration = 1000000000000000000000):
    if search_pattern is None:
        search_pattern = p2root+'/'+folder+'/*.'+ext
    wavs = glob.glob(search_pattern, recursive=True)
    print("Found",len(wavs),"files in folder",folder)
    ...
    for line in (lines):
        ...
        # print("/".join(line["audio_path"].split("/")[-2:]))
        if "/".join(line["audio_path"].split("/")[-3:]) not in wavs:
            continue
        file, trans = line['audio_path'], normalize(line['text'])
        file = "/".join(file.split("/")[-3:]).replace(ext,"")
...
if __name__ == "__main__":
    ...
    for test_folder in tqdm.tqdm(os.listdir(p2root)):
        ...
        for folder in tqdm.tqdm(os.listdir(p2root)):
            ...
            if folder == test_folder:
                continue
            wavs, samples, root, wav2trans = parse_folder(folder)
